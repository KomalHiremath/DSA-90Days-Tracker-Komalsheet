...
void VectorsIterators(){
    ...
    it=it+2;                                                            // moves by 2 indices pos
    cout << (*it) << " ";
    cout<<"\n"; 
...
void VectorsDeletion(){
    ...
    v1.erase(v1.begin()+2, v1.begin()+4);  // always include the pos of next elem(deleting) as shown in v1.begin()+4)  [start, end)
        for (auto j:v1){
        cout<<j<<" ";
    }
}
...
void Map(){ // SORTED KEY(✔️) UNIQUE KEY(✔️)   UNIQUE VAL(MAY/MAY NOT)

//map<key,value>m
 map<int,int>m;
 m[1]=2;
 m.emplace(9,3);
 m.insert({5,4}); // 1 2 , 5 4 , 9 3 ; the keys 1 9 5 are sorted
 for(auto val : m){
    cout<< val.first <<" " << val.second <<" , "; // val here is of type pair<const int, int>, and cout << val doesn't know how to print a pair directly.So we wrote 1st & 2nd
 }cout<<endl;

// map<pair<key,key>,val>mp
 map<pair<int,int>,int>mp;
 mp[{7,2}]=10;
 mp[{6,18}]=1;
 for(auto res : mp){
    cout<< res.first.first <<" " << res.first.second <<" "<< res.second << endl; // 6 18 1 ; 7 2 10
}
cout<< m[1] <<endl;// gives th key's value --> 2
...
if (it != mp.end()) {
    ...
    cout << "Key not found" << endl; // Value: 10
}
}

void Multimap() {// SORTED KEY(✔️) UNIQUE KEY(❌)   UNIQUE VAL(MAY/MAY NOT)
    // everything same as map, only it can store multiple duplicate keys
    // does not support element access using [] like: mpp[key] = value;
}

void UnorderedMap(){ // SORTED KEY(❌) UNIQUE KEY(✔️)   UNIQUE VAL(MAY/MAY NOT)
    // rest all fun is same as MAP
    }
...
void Sort(){
    //INDEX= 0 1 2 3  
    int a[]={1,2,5,3};
    sort(a+2,a+4); // SAME AS VECTOR [START, END)
    {
        for (int i=0; i<4; i++){
            cout<<a[i] <<" ";   // 1 2 3 5 
        }
    ...
    sort(a, a+4, greater<int>()); // greater<int>() --> prints in descending order; its a COMPARATOR
      {
        for (int i=0; i<4; i++){
            cout<<a[i] <<" ";   // 1 2 3 5 
        }
...
/*                  bool comp(...);      // comparator or helper
                    void solve();        // logic
                     int main();         // entry point                *** VERY VERY IMP ***
*/

// QUE: sort it according to second element
// if second element is same, then sort it according to first element but in descending

 bool comp(pair<int,int>p1, pair<int,int>p2){
    if(p1.second<p2.second) return true;
    if(p1.second>p2.second) return false;
    // 3rd condition is if p1.second = p2.second then 
    //in question they said to sort the first element but in descending, therefore
   ...
   void SortQue(){
    ...
    sort(a, a+3, comp);
    for (int i=0;i<3; i++){
    cout<< a[i].first<< "," << a[i].second << "  "; // 4,1  2,1  1,2 
      }
  }

void MaxMinElements(){
    int m []={1,4,8,2};
    int maxi=*max_element(m, m+4); // 8
    int min=*min_element(m, m+4);// 1
    cout << maxi << endl; // maxi when: to use or print just the max value ||  m when: to access or print all elements
     cout << min ;               
...
/*  __builtin_popcount(num) --> Returns the number of set bits (1s) in integer num.
...
int main(){
long long num=28378835482387; //26
...
next_permutation(s.begin(), s.end()) --> strings to be SORTED
...
//    **** If you see "not declared in this scope" , it's usually a missing forward declaration or a function written after its usage. ****
